...
var Extractor = (function () {
    ...
    Extractor.prototype.extractHtml = function (filename, src) {
        ...
        $('*').each(function (index, n) {
            ...
            if (node[0].prev && node[0].prev.prev) {
                if (node[0].prev.prev.type === 'comment' && node[0].prev.prev.data.match(/^ \/\/\/ .*/)) {
                    parentComment = node[0].prev.prev;
                }
            }
            for (var attr in node.attr()) {
                if (attr === 'translate') {
                    ...
                    if (parentComment) {
                        extractedComment = parentComment.data.replace(/^ \/\/\/ /, '');
                        parentComment = null;
                    }
